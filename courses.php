...
if(isset($newtrainer)) echo icon('ok','Поздравляем! Вы успешно стали тренером');
...
concat('<nobr><span class=status_',v.StatusID,' title=\"',st.Name_$lang;,'\" align=absmiddle></span> ',st.Name_$lang;,'</nobr>') Status
 from ut_materials v join ut_statuses st on st.statusid=v.statusid where v.ID='$id'");
	...
	<button class='btn-green .matBut' style='margin-top:30px' type='submit' onClick="document.location.href='?act=update&id=<?=$id?>&form_ok=1'">Отправить на модерацию</button>
